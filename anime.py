...
class Anime:
    ...
    def __init__(self, path):
        ...
        if len(self.full_paths) == 0:
            console.print(
            """[b][red]No directories matching the scan format were found.\n[yellow]Learn more about directory formatting: [blue]https://github.com/AbhiramH427/AniName#pre-requisite\n""")
            os.system('pause')
            sys.exit()
        ...
        for i, id in enumerate(self.anime_dirs):
            ...
            anime_display_title = utils.format_season(anime_titles[i], int(season), int(part), True)
